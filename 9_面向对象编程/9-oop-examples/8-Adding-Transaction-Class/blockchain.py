...
# hence this is our identifier (e.g. for sending coins)
owner = 'Max'
...
def valid_proof(transactions, last_hash, proof):
    ...
    # - this would take significantly longer (and this allows you to control 
    # the speed at which new blocks can be added)
...
def get_balance(participant):
    ...
    print('Printing sender transactions when getting Max\'s balance: {}'.format(tx_sender))
    ...
    # be able to spend coins before the transaction was confirmed
    #  + included in a block
...
def mine_block():
    ...
    proof = proof_of_work()
    # print('Printing Mining Proof:{}'.format(proof))
...
def verify_chain():
    ...
    for (index, block) in enumerate(blockchain):
        # print('Verifying chain: {}, {}'.format(index, block.__dict__))
        if index == 0:
            continue
        if block.previous_hash != hash_block(blockchain[index - 1]):
            return False
        if not valid_proof(block.transactions[:-1], block.previous_hash, block.proof):
            # print('When chain verification fails, echoing block.proof: {}'.format(block.proof))
